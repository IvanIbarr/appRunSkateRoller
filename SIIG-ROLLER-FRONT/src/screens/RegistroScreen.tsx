...
export const RegistroScreen: React.FC<RegistroScreenProps> = ({
    ...
    if (Platform.OS === 'web') {
      if (!profilePhotoInputRef.current) {
        ...
        input.onchange = async (e: Event) => {
          ...
          if (file) {
            ...
            try {
              ...
              try {
                ...
                reader.readAsDataURL(workingFile instanceof File ? workingFile : new Blob([workingFile], {type: 'image/jpeg'}));
              } catch {
          ...
          if (profilePhotoInputRef.current) {
            (profilePhotoInputRef.current as HTMLInputElement).value = '';
          }
        };
          ...
          ImagePicker.launchCamera(
            ...
            (response: any) => {
              ...
              if (asset?.base64) {
                ...
                setProfilePhotoPreview(dataUri);
                setFormData(prev => ({
                  ...prev,
                  fotoPerfil: dataUri,
                  avatar: null,
  ...
  const handleRemoveProfilePhoto = () => {
    ...
    if (Platform.OS === 'web' && profilePhotoInputRef.current) {
      (profilePhotoInputRef.current as HTMLInputElement).value = '';
    }
  };
  ...
  const handleProfileModeChange = (mode: 'avatar' | 'foto') => {
    ...
    setFormData(prev => ({
      ...prev,
      avatar: mode === 'avatar' ? prev.avatar : null,
      fotoPerfil: mode === 'foto' ? prev.fotoPerfil : null,
    }));
    if (mode === 'avatar') {
      setProfilePhotoPreview(prev => {
        if (prev && prev.startsWith('blob:')) {
    ...
    try {
      ...
      if (response.success && response.usuario) {
        ...
        if (formData.fotoPerfil) {
          try {
            const photoResponse = await authService.updateProfilePhoto(formData.fotoPerfil);
            if (!photoResponse.success) {
              console.warn('No se pudo guardar la foto de perfil:', photoResponse.error);
            }
          } catch (photoError) {
            console.error('Error al guardar foto de perfil:', photoError);
          }
        }
    ...
    <KeyboardAvoidingView
            ...
            <View style={styles.pickerContainer}>
              ...
              {profileMode === 'avatar' ? (
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}>
                  {(profilePhotoPreview || formData.fotoPerfil) ? (
                    <Image
                      source={{uri: profilePhotoPreview || formData.fotoPerfil}}
              ...
              {profileMode === 'foto' && formData.fotoPerfil ? (
                <TouchableOpacity
      ...
      <AvatarSelector
        ...
        onSelectAvatar={(avatar) => {
          setFormData({...formData, avatar, fotoPerfil: null});
